...
bool ESP32_STM_UART::check_wifi(){
    while (WiFi.status() != WL_CONNECTED && retries > 0) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
            ...
            print_debug(debug_flag, String("Wi-Fi not connected after all retries!"));
        } else {
            print_debug(debug_flag, String("Wi-Fi not connected! Retrying..."));
        }
    }
    return WiFi.status();
}
    ...
    while (millis() - startTime < 70000) {
        if (_serial.available()) {
            ...
            if (cmd == SEND_WEIGHT_CMD_TRANSMIT_WEIGHT_ARRAY) {                
                print_debug(debug_flag, String("Data read from UART started."));
                _serial.readBytes(cowId, 6);
                print_debug(debug_flag, String("CowId read from UART finish."));
                _serial.readBytes((uint8_t *)&ts, 4);
                print_debug(debug_flag, String("Time read from UART finish."));
                for (size_t i = 0; i < weightCount; i++) {
                    _serial.readBytes((uint8_t *)&weight[i], 4);
                }
                for (size_t i = 0; i < weightCount; i++) {
                    print_debug(debug_flag, weight[i]);
                }
                
                print_debug(debug_flag, String("Weight_array read from UART finish."));
                print_debug(debug_flag, String("Data read from UART ended. Sending to server..."));
                ...
                print_debug(debug_flag, String("Wrong message!") + String(cmd, HEX));
                return sendWeightRsp(0x02);}
...
bool ESP32_STM_UART::sendDataToServer(uint8_t *cowId, uint32_t ts, float *weight, uint8_t weightCount) {
    ...
    const int cowIdSize = 6;
    char cowIdStr[2 * cowIdSize + 1];
    for (int i = 0; i < cowIdSize; i++) {
        sprintf(&cowIdStr[i * 2], "%02X", cowId[i]);
    }
